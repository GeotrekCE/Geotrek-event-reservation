...
def query_stats_bilan(params):
    ...
    taux_remplissage_global = round(sum_nb_participants / sum_events_capacity, 3) if sum_events_capacity else 0
    ...
    taux_remplissage_moyen = (
        ...
        / nb_events_capacity
    ) if nb_events_capacity > 0 else 0
    taux_remplissage_moyen = (
    ...
    # Animations passées
    events_passe = [e for e in events if (e.end_date or e.begin_date) < datetime.now().date()]
    nb_events_passe = len(events_passe)

    # Animations passées avec réservation
    events_capacity_passe = [e for e in events_capacity if (e.end_date or e.begin_date) < datetime.now().date()]
    ...
    taux_remplissage_global_passe = round(sum_nb_participants_passe / sum_events_capacity_passe, 3) if sum_events_capacity_passe else 0
    ...
    taux_remplissage_moyen_passe = (
        sum(
            [
                d.sum_participants / d.capacity
                for d in events_capacity_passe
            ]
        )
        / nb_events_capacity_passe
    ) if nb_events_capacity_passe > 0 else 0
    taux_remplissage_moyen_passe = (
