...
def query_stats_bilan(params):
    ...
    if 'year' in params:
        query = query.filter(func.date_part('year', GTEvents.begin_date) == params['year'])
    nb_events = query.count()
    events = query.all()
    sum_nb_participant = sum([d.sum_participants for d in events])
    sum_clean_nb_participants = sum([d.clean_nb_participants for d in events])
    # Taux de remplissage de toutes les animations
    taux_remplissage = sum([
            d.sum_participants / (d.clean_nb_participants if d.clean_nb_participants > 0 else 1 )
            for d in events
        ]) / nb_events
    # Taux de remplissage des animations passées
    taux_remplissage_passe = sum([
        d.sum_participants / (d.clean_nb_participants if d.clean_nb_participants > 0 else 1 )
        for d in events
        if (d.end_date or datetime.now().date()) < datetime.now().date()
    ]) / nb_events

    query = db.session.query(
        func.count(TAnimationsBilans.id_bilan)
    ).filter(
        TAnimationsBilans.annulation == True
    ).join(
        GTEvents, GTEvents.id == TAnimationsBilans.id_event
    )
    if 'year' in params:
        query = query.filter(func.date_part('year', GTEvents.begin_date) == params['year'])
