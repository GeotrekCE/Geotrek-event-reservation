  ...
  <div class="flex min-h-full flex-col">
    ...
    <main class="flex flex-grow gap-x-4">
              ...
              <router-link
                ...
                :class="{
                  ...
                  'bg-red-200 border-red-500':
                    data.cancelled && data.id === selectedEventId,
                }"
                  ...
                  <span>
                    <i
                      ...
                      :class="{
                        ...
                        'pi-times-circle text-orange-400': !data.published,
                      }"
                    />
                    {{ data.name }}
                  </span>
      ...
      <section class="w-full md:w-3/4" v-if="selectedEvent">
        <div
          class="md:hidden h-12 flex items-center cursor-pointer"
        ...
        <p-card class="rounded-sm md:mt-8 md:mr-2 mx-auto">
          <template #title>
            <div class="md:flex items-center justify-between space-y-4">
              {{ selectedEvent.name }}
              <div
                class="text-center md:ml-auto text-sm font-medium mt-4 md:mt-0"
              >
                <a
          ...
          <template #content>
            <div
              v-if="selectedEventCanceled === true"
              class="bg-red-300 max-w-xl mx-auto mb-12 p-2"
            >
            ...
            <div class="my-4">
              ...
              <div v-if="selectedEvent.bookable">
                <div>
                  ...
                  <span v-html="selectedEvent.capacity"></span>
                </div>
                <div>
                  <strong> Places restantes: </strong
                  ><span>{{
                    selectedEvent.capacity - selectedEvent.sum_participants
                  }}</span>
                </div>
              </div>
            </div>
            ...
            <Tabs>
              <TabList>
                <Tab value="tab-resa">Réservations</Tab>
                <Tab value="tab-summary">Résumé / RDV</Tab>
                <Tab value="tab-bilan">Bilan</Tab>
              </TabList>
              <TabPanels>
                <TabPanel value="tab-resa">
                  <reservation-progress
                    class="my-4"
                  ...
                  <div class="my-4 text-center">
                    <button
                      ...
                      v-if="
                        reservationOpened.value && statutReservation === 'list'
                      "
                ...
                <TabPanel value="tab-summary">
                  <div>
                    <h2
                      class="text-base mt-4 font-medium leading-7 text-gray-900"
                    >
                      Informations pour le rendez-vous
                    </h2>
                    <p class="mt-1 text-sm leading-6 text-gray-600">
                      Ces informations sont utiles lors de l'envoi de l'email
                      aux utilisateurs. Ces informations n'apparaissent pas dans
                      Geotrek.
                    </p>
                ...
                <TabPanel value="tab-bilan">
                  ...
                  >
                    <p class="ml-4">
                      La saisie du bilan est réservée à l'animateur
                    </p>
                  </p-message>
                  ...
                  <div v-if="selectedEventCanceled">
                    ...
                    >
                      <p class="ml-4">
                        L'animation a été annulée, il n'y a pas de bilan à
                        saisir
                      </p>
                    </p-message>
                  </div>
                ...
                <TabPanel>
                  ...
                  <p-message severity="warn" :closable="false">
                    <div class="space-y-4 ml-4">
                      <p v-if="selectedEvent.cancelled == false">
                        L'annulation d'une animation doit d'abord être faite
                        dans GeoTrek.
                      </p>

                      <p>
                        Une fois l'animation annulée dans GeoTrek, le formulaire
                        ci dessous va déclencher l'envoi d'un mail à tous les
                        inscrits pour leur préciser l'annulation de l'animation.
                      </p>
                    </div>
                  </p-message>
      ...
      <section
        class="hidden md:block md:w-3/4 2xl:w-4/5 grid min-h-full px-6 py-24 sm:py-32 lg:px-8"
        v-else
      >
...
import {
  getDistricts,
  getTouristiceventType,
  getTouristicEventDetail,
} from "@/utils/gta_api";
...
const selectedEventCanceled = computed(
  () => selectedEvent.value?.cancelled === true,
);
const selectedEventBilanCanceled = computed(
  () => selectedEvent.value?.bilan?.annulation === true,
);
...
const reservationOpened = computed(() =>
  isReservationOpened(selectedEvent.value),
);
...
function onEditReservation(id_reservation: number) {
  const currentResa = resas.value.results?.find(
    (r: Resa) => r.id_reservation === id_reservation,
  );
...
async function onSaveInfos() {
  ...
  try {
    selectedEventInfoRDV.value = await putEventInfo(selectedEvent.value.id, {
...
async function onSubmitReservation({
  liste_attente,
  ...values
}: Partial<Resa>) {
  saving.value = true;
...
async function loadSelectedEvent() {
  ...
  selectedEventSummary.value = {
    ...
    sum_participants_moins_6_ans:
      selectedEvent.value.sum_participants_moins_6_ans,
    ...
    sum_participants_plus_12_ans:
      selectedEvent.value.sum_participants_plus_12_ans,
  };
}
