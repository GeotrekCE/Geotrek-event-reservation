<template>
  <div>
    ...
    <main class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8 px-4 py-6" v-else>
      <section class="border-b border-gray-900/10 pb-12" v-if="loadingEvent">
        Chargement en cours... Merci de patienter...
      </section>

      <section class="border-b border-gray-900/10 pb-12" v-else-if="eventError">
        {{ eventError }}
      </section>
      ...
      <template v-else-if="status !== STATUS.SUCCESS">
        <section class="border-b border-gray-900/10 pb-12">
          <p>
            ...
            <span class="font-medium" v-if="event"
              >{{ event.name }} ({{ formatDateString(event.begin_date) }})</span
            >
